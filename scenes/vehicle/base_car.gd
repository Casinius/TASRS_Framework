...
#		clutch_input = 1.0
	...
	# 换档瞬间扭矩大幅削减，防止过冲
	var gear_changed = (drivetrain.selected_gear != last_gear)
	if gear_changed:
...
# ... 原有的 torque_out 计算 ...
	...
	if shift_timer > 0:
		torque_out *= SHIFT_TORQUE_REDUCTION
		shift_timer -= delta
